...
Entries marked **🚧** are not yet declared in `secretspec.toml` (the file
ships 23 of these rows; expanding to 49 is one of the open
follow-ups). Entries marked **✅** are declared and resolve through the
...
| | `OPENAI_ORG_ID` | `env://OPENAI_ORG_ID` | `secrets.ai.yaml#openai_org_id` | `onepassword://Homelab/AI` | 🚧 pending declaration |
| | `HUGGINGFACE_TOKEN` | `env://HUGGINGFACE_TOKEN` | `secrets.ai.yaml#huggingface_token` | `onepassword://Homelab/AI` | 🚧 pending declaration |
| | `STABILITY_API_KEY` | `env://STABILITY_API_KEY` | `secrets.ai.yaml#stability_api_key` | `onepassword://Homelab/AI` | 🚧 pending declaration |
| | `REPLICATE_API_TOKEN` | `env://REPLICATE_API_TOKEN` | `secrets.ai.yaml#replicate_api_token` | `onepassword://Homelab/AI` | 🚧 pending declaration |
...
| | `NIX_PACKAGES_CACHE_TOKEN` | `env://NIX_PACKAGES_CACHE_TOKEN` | `secrets.ci.yaml#nix_packages_cache_token` | `env://` | 🚧 pending |
...
| | `CLOUDFLARE_EMAIL` | `env://` | `secrets.cloud.yaml#cloudflare_email` | `onepassword://Homelab/Cloud` | 🚧 pending |
| | `CLOUDFLARE_ZONE_ID` | `env://` | `secrets.cloud.yaml#cloudflare_zone_id` | `onepassword://Homelab/Cloud` | 🚧 pending |
| | `TAILSCALE_AUTH_KEY` | `env://` | `secrets.cloud.yaml#tailscale_auth_key` | `onepassword://Homelab/Cloud` | ✅ declared |
| | `HOMELAB_DUCKDNS_TOKEN` | `env://` | `secrets.cloud.yaml#duckdns_token` | `keyring://` (per-host) | 🚧 pending |
| | `ACME_ACCOUNT_KEY` | `env://` | `secrets.cloud.yaml#acme_account_key` | `onepassword://Homelab/Cloud` | 🚧 pending |
...
| | `RESTIC_REPO_PASSWORD` | `env://` | `secrets.storage.yaml#restic_repo_password` | `onepassword://Homelab/Storage` | 🚧 pending |
| | `RCLONE_CONFIG_PASS` | `env://` | `secrets.storage.yaml#rclone_config_passphrase` | `onepassword://Homelab/Storage` | 🚧 pending |
...
| | `KUBEADMIN_PASSWORD` | `env://` | `secrets.kubernetes.yaml#kubeadmin_password` | `vault://` via astral-key | 🚧 pending |
| | `K3S_KUBELET_TOKEN` | `env://` | `secrets.kubernetes.yaml#k3s_kubelet_token` | `vault://` via astral-key | 🚧 pending |
...
| | `ETHEREUM_WALLET_ADDRESS` | `env://` | `secrets.mining.yaml#ethereum_wallet_address` | `onepassword://Homelab/Mining` | 🚧 pending |
| | `MONERO_WALLET_ADDRESS` | `env://` | `secrets.mining.yaml#monero_wallet_address` | `onepassword://Homelab/Mining` | 🚧 pending |
...
| | `LOKI_INGEST_TOKEN` | `env://` | `secrets.monitoring.yaml#loki_ingest_token` | `vault://` via astral-key | 🚧 pending |
| | `UPTIME_ROBOT_API_KEY` | `env://` | `secrets.monitoring.yaml#uptime_robot_api_key` | `onepassword://Homelab/Monitoring` | 🚧 pending |
| | `GRAFANA_OAUTH_CLIENT_SECRET` | `env://` | `secrets.monitoring.yaml#grafana_oauth_client_secret` | `vault://` via astral-key | 🚧 pending |
...
| | `ANSIBLE_VAULT_PASSWORD` | `env://` | `secrets.automation.yaml#ansible_vault_password` | `onepassword://Homelab/Automation` | 🚧 pending |
| | `HOMELAB_RUNNER_REG_TOKEN` | `env://` | `secrets.automation.yaml#act_runner_reg_token` | `env://` | 🚧 pending |
...
| | `JELLYFIN_API_KEY` | `env://` | `secrets.selfhosting.yaml#jellyfin_api_key` | `onepassword://Homelab/SelfHosting` | 🚧 pending |
| | `PAPERLESS_API_TOKEN` | `env://` | `secrets.selfhosting.yaml#paperless_api_token` | `onepassword://Homelab/SelfHosting` | 🚧 pending |
| | `PHOTOPRISM_ADMIN_PASSWORD` | `env://` | `secrets.selfhosting.yaml#photoprism_admin_password` | `onepassword://Homelab/SelfHosting` | 🚧 pending |
| | `MEALIE_API_KEY` | `env://` | `secrets.selfhosting.yaml#mealie_api_key` | `onepassword://Homelab/SelfHosting` | 🚧 pending |
| | `VAULTWARDEN_SMTP_PASSWORD` | `env://` | `secrets.selfhosting.yaml#vaultwarden_smtp_password` | `vault://` via astral-key | 🚧 pending |
| | `NETWORK_WIFI_PASSWORD` | `env://` | `secrets.selfhosting.yaml#network_wifi_password` | `keyring://` (per-host, low risk) | 🚧 pending |

**Totals:** 49 entries, 23 declared in `secretspec.toml` (✅), 26 pending
declaration (🚧). The 3 development-profile rows for AI API keys
(`NVIDIA_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) reuse the
default-profile names — they're overrides, not new secrets.

By-category ✅ count: aiServices 3, ci 2, cloud 3, storage 3,
kubernetes 2, mining 4, monitoring 2, automation 2, selfHosting 2
(= 23). By-category 🚧 count: aiServices 4, ci 1, cloud 4, storage
2, kubernetes 2, mining 2, monitoring 3, automation 2, selfHosting
6 (= 26).
...
- `secretspec.toml` (the 23 declared entries)
